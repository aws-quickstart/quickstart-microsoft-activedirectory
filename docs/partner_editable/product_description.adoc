...
This Quick Start is for organizations running workloads in the AWS Cloud that require secure, low-latency connectivity to AD DS and DNS services. After reading this guide, IT infrastructure personnel should have a good understanding of how to design and deploy a solution to launch AD DS in the AWS Cloud, or extend their on-premises AD DS into the AWS Cloud. The Quick Start optionally deploys a one- or two-tier https://aws-quickstart.github.io/quickstart-microsoft-pki/[Microsoft Public Key Infrastructure].
